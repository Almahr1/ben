...
void test_save_and_load_file(void) {
    ...
    // Initialize a local editor buffer for testing
    TextBuffer test_buffer;
    init_editor_buffer(&test_buffer);
    
    Line *line1 = create_new_line("Hello world!");
    test_buffer.current_line_node = line1;
    insert_line_at_end(&test_buffer, line1);
    Line *line2 = create_new_line("This is a test file.");
    insert_line_at_end(&test_buffer, line2);
    Line *line3 = create_new_line("");
    insert_line_at_end(&test_buffer, line3);
    Line *line4 = create_new_line("The end.");
    insert_line_at_end(&test_buffer, line4);

    // Save the buffer to a file
    saveToFile(TEST_FILENAME, &test_buffer);

    // Free the current buffer and re-initialize it for the load test
    free_editor_buffer(&test_buffer);
    init_editor_buffer(&test_buffer);

    // Load the file into the buffer
    loadFromFile(TEST_FILENAME, &test_buffer);
    
    // Assert that the content and number of lines are correct
    ASSERT_EQ(4, test_buffer.num_lines, "Loaded buffer should have 4 lines");
    
    Line* current_line = test_buffer.head;
...
void test_load_empty_file(void) {
    ...
    ASSERT_EQ(1, test_buffer.num_lines, "Loaded buffer should have 1 line for an empty file");
    ASSERT_NOT_NULL(test_buffer.head, "Head should not be NULL");
    ASSERT_NOT_NULL(test_buffer.current_line_node, "Current line node should not be NULL");
    
    char* content = line_to_string(test_buffer.head);
