...
int is_line_valid_in_buffer(TextBuffer *buffer, Line *target_line) {
    if (!buffer || !target_line) return 0;
...
int can_undo(void) {
    if (undo_stack.count <= 0 || undo_stack.current < 0) return 0;
...
int can_redo(void) {
    if (undo_stack.current >= undo_stack.count - 1) return 0;
...
void validate_cursor_position(TextBuffer *buffer) {
    ...
    if (!buffer->current_line_node || !is_line_valid_in_buffer(buffer, buffer->current_line_node)) {
            ...
            if (second_line) {
                ...
                if (second_content) {
                    ...
                    if (buffer->current_line_node == second_line) {
                        buffer->current_line_node = target_line;
                        buffer->current_col_offset = op->col_pos + buffer->current_col_offset;
                    }
                    
                    // Merge the lines back together
                    line_insert_string_at(target_line, op->col_pos, second_content);
                    free(second_content);
